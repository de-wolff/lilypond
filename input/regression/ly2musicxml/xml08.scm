...
(display "testing xml:createTextNode:" info-port)
...
(define node (xml:createTextNode doc "this is text"))
;; there is in xpath no way to distinguish between CDATA and text
;; so we check for not having the specific CDATA text
(display "\\<root>[[:space:]]*[a-z ]*[[:space:]]*[<]/root>" regex-port)

(xml:appendChild root node)

; check node
(display " and (//text()='this is text')" xpath-port)
